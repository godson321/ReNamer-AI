...
## ✅ 2026-03-12 修复记录（Win32 新表格文件列表空白）

**问题现象**：启用 Win32 新表格后，文件数存在但行文本不显示。  
**根因**：Unicode ListView 使用场景下，通知码使用了 ANSI 常量，导致取文本回调未触发。  
**修改点**：  
- Win32Interop.cs：补齐/区分 A/W 通知常量。  
- Win32FileListViewHost.cs：通知分发同时兼容 A/W。  
**验证结果**：dotnet build 通过；Win32 引擎下列表文本恢复显示；排序/勾选/表头交互正常。  
**影响范围**：仅 Win32 文件列表路径，WPF DataGrid 路径无变更。  
...
**Public Interfaces / Types**：无公共 API 变更，仅内部 Win32 互操作与通知分发实现修复。  

**Test Plan**：  
- 文档正确性：标题、日期、根因、修改点、验证结论五项齐全  
- 内容一致性：与代码修复点一一对应（2 个文件）  
- 可追溯性：记录可直接指导复现与回归验证  

**Assumptions**：采用增量记录而非重写整篇文档；日期统一使用 2026-03-12；文档口径为“实现验证报告”风格（问题→根因→改动→验证）。  
